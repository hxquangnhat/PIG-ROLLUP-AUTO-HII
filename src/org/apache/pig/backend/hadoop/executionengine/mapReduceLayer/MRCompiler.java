...
public class MRCompiler extends PhyPlanVisitor {
    ...
    public MROperPlan compile() throws IOException, PlanException,
        ...
        for (MapReduceOper check : MRPlan)
            if (PlanHelper.getPhysicalOperators(check.reducePlan,
                ...
                check.mapPlan.replace(pfe, pfe2);
                POUserFuncRollupSample puf = (POUserFuncRollupSample)pfe2.getInputPlans().get(0).getLeaves().get(0);                List<PhysicalOperator> pufIP = puf.getInputs();
                for (int i = 0; i < pufIP.size(); i++) {
                    POProject po = (POProject)pufIP.get(i).getInputs().get(0);
                    po.setColumn(i);
                }
    ...
    private void doAutoRollupJob(MapReduceOper check) throws IOException,
        ...
        if (lpre.get(0) instanceof POLoad) {
            ...
            check.markRollupSampler();
            //Add the rollup job below this (sampling) job.
            doAddRollupJob(pre, fSpec, check);
        ...
        else 
        {
            ...
            //Create rollup job.
            doAddRollupJob(null, fSpec, newplan);
     ...
     * @throws CloneNotSupportedException
     */
    private void doAddRollupJob(POLoad pre, FileSpec fSpec, MapReduceOper prevJob)
        ...
        MapReduceOper rollup = getMROp();
        rollup.requestedParallelism = prevJob.requestedParallelism;
        rollup.runtimeParallelism = prevJob.runtimeParallelism;
        rollup.mapPlan = prevJob.mapPlan.clone();
        rollup.mapPlan.add(pl);
        MRPlan.add(rollup);
        MRPlan.connect(prevJob, rollup);

        //Set custom partitioner
        rollup.customPartitioner = ROLLUP_AUTO_PIVOT_PARTITIONER;

        List<POForEach> nlfe = PlanHelper.getPhysicalOperators(rollup.mapPlan,
                POForEach.class);

        for (POForEach tmpfe : nlfe) {
            if (tmpfe.getAlias().equals("cube")) {
                rollup.mapPlan.trimAbove(tmpfe);
                break;
            }
        }
        ...
        for (POForEach tmpfe : nlfe) {
            if (tmpfe.getAlias().equals("cube")) {
                rollup.mapPlan.connect(pl, tmpfe);
                break;
            }
        }
        ...
        PhysicalOperator tmp2 = tmp.clone();
        PhysicalOperator tmp3 = null;
        rollup.reducePlan.add(tmp2);
        List<PhysicalOperator> succ = prevJob.reducePlan.getSuccessors(tmp);
        while (succ != null) {
            tmp = succ.get(0);
            tmp3 = tmp.clone();
            rollup.reducePlan.add(tmp3);
            rollup.reducePlan.connect(tmp2, tmp3);
            tmp2 = tmp3;
            succ = prevJob.reducePlan.getSuccessors(tmp);
        }
...
//        FileSpec newStore = new FileSpec("/tmp/sampling", null);
        ...
        prevJob.setReduceDone(true);
        rollup.markGroupBy();
        rollup.setReduceDone(true);
        //mark this job as a rollup job
        rollup.markRollup();
        
        POForEach pfe = PlanHelper.getPhysicalOperators(rollup.mapPlan, POForEach.class).get(0);
        POLocalRearrange plr = PlanHelper.getPhysicalOperators(rollup.mapPlan, POLocalRearrange.class).get(0);
        ...
        rollup.mapPlan.replace(pfe, pfe2);
        POUserFuncRollupSample puf = (POUserFuncRollupSample)pfe2.getInputPlans().get(0).getLeaves().get(0);
        List<PhysicalOperator> pufIP = puf.getInputs();
        for (int i = 0; i < pufIP.size(); i++) {
            POProject po = (POProject)pufIP.get(i).getInputs().get(0);
